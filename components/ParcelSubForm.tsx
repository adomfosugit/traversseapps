...
import { UpdateJobLawyerReport1, UpdateOathofProof, UpdateParcelSub, UpdatePurchaseStage, uploadDoc } from "@/lib/Appwrite/api"
...
export function ParcelSubForm({ JobProjectID }: { JobProjectID: string }) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const updateResult = await UpdateParcelSub(JobProjectID, fileUrl)

      if (updateResult) {
        toast({
          title: "Upload Successful",
          description: "Parcel document uploaded successfully",
        })
        form.reset()
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-xl shadow-lg border">
      <div className="mb-6">
        ...
        <p className="text-gray-600 text-sm">
          Upload your Parcel form document
        </p>
      </div>
            ...
            render={({ field: { onChange, ...rest } }) => (
              <FormItem>
                ...
                <FormDescription className="text-sm text-gray-600">
                  Upload a PDF document or image file containing the Parcel form.
