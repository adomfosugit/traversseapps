...
import { updateLandProjectSiteVisit, UpdateUserLandProceedings, UpdateUserLandProceedingsRegister } from '@/lib/Appwrite/api'
...
const RegistrationPaymentCard = ({ name, email, land, project, agreedPrice  }: Props) => {
  ...
  const baseServices = [
    { label: 'Stamp Duty', price: 1000.00 },
    { label: 'Processing of Consent/Concurrence', price: 3500.00 },
    { label: 'Land Title Registration', price: 10000 },
  ]
  ...
  const componentProps = {
    ...
    onSuccess: async () => {
      try {
        const result = await  UpdateUserLandProceedingsRegister(project)
